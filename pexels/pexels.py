...
def get_image_links(category:str,n_images:int,authcode:str):
    ...
    response = requests.get("https://api.pexels.com/v1/search?query="+category+"&per_page="+str(n_images),headers= my_headers)
...
def download_image(output_loc:str,category:str,image_link:str):
    if not os.path.exists(os.path.join(output_loc,category)):
        os.makedirs(os.path.join(output_loc,category))
    image_path = os.path.join(output_loc,category,image_link.rsplit("/",1)[-1].rsplit("?",1)[0])
    response = requests.get(image_link, stream=True)
    with open(image_path, 'wb') as outfile:
        outfile.write(response.content)

def download_pexel_images(auth_code:str,output_loc:str,n_images_per_class:int,categories:str):
    """
    ...
    try:
        ...
        for image_link in flat_list:
            download_image(output_loc,image_link[1],image_link[0])
    except Exception as error:
        print(error)
